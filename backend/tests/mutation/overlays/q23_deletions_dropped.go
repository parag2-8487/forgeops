...
// which substitutes this file for `agent/internal/fileops/fileops.go` for the duration of one
// test run. It is never compiled into the agent.
//
// Byte-for-byte the committed `fileops.go` except for one branch in `UnifiedDiff`: deletion
// lines are no longer emitted. Appendix B's prescribed control is "compile change items with
...
//
// This file is GENERATED by the one-shot script recorded in the Q-23 commit, so it is faithful
// to its original rather than hand-transcribed.
//
// With this applied, TestPropertyQ23_UnifiedDiffCapturesEveryEditLosslessly must FAIL: context
// plus deletions stop reconstructing the before text as soon as any line is removed.
