...
from src.secrets.redaction import RedactedChunk, create_redacted_chunk, create_redacted_instruction
...
def test_model_endpoint_receives_no_secrets():
    ...
    prompt = assemble_prompt(system=create_redacted_chunk("sys"), chunks=[], instruction=inst)
    req = CompletionRequest(model="gpt-4", messages=[{"role": "user", "content": prompt}])
