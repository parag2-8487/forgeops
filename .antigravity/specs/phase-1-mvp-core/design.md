...
**Corrected for Phase 2.** This paragraph used to end "Nothing in this catalogue performs a deployment, drives a dashboard, interprets natural language, collects metrics, remediates automatically, or records feedback for learning — the six capabilities `phases.md` excludes verbatim." That was true of Phase 1 and `deployment.apply_manifests` makes the first clause false: the catalogue now performs a deployment, deliberately, as `phases.md` §2.2's first box. The other five remain absent — no dashboard is driven from here, no natural language is interpreted, no metric is collected, nothing remediates automatically, and no feedback is recorded. Leaving the original sentence in place would have been a statement this document no longer supports, which is worse than an amended one.
