  ...
  - [x] 20.11 Verify criterion 11 — coverage ≥ 70 % per component
      ...
      - **Unchecked 2026-08-21.** This was `[x]` while two of the three gates did not exist. They exist now and were run: backend 87.06 %, agent 78.8 %, frontend 37.04 % — so the verification has been performed and its outcome is negative.
        - **Still open 2026-08-26, and the reason is now narrower.** The backend gate passes with real margin: 85.52 % over 2405 tests, enforced by `--cov-fail-under=70` in `backend/pyproject.toml`'s addopts so it applies to every pytest run rather than only to CI. The FRONTEND gate is what keeps this unticked — last measured at 37.04 %, far below 70 — so the criterion is not met and ticking it would be exactly the false claim the 2026-08-21 note removed. Raising frontend coverage is real work, not a tracking fix.
          - **Ticked 2026-08-27, and the 37.04 % above was stale.** Re-measured all three, each the way its OWN gate measures it, because an aggregate would let one component hide behind another: **backend 85.77 %** (2557 passed, `--cov-fail-under=70`), **agent 76.7 %** (`scripts/check-coverage.sh 70`, one merged profile with `-coverpkg=./internal/...`, reporting `ok: agent coverage 76.7% meets the 70% gate`), **frontend 95.81 %** statements / 85.34 % branch / 95.47 % functions / 95.92 % lines (`vitest --coverage`, 20 test files). All three clear 70 %, so the criterion is met on its own terms.
            - The record was stale in BOTH directions. Frontend was never 37 % at this commit — it measured 90.58 % before any work in this session, and the gate was already wired as `pnpm exec vitest --run --coverage` in `ci.yml`'s frontend job. Agent was recorded as 78.8 % and measures 76.7 %; the earlier figure was not produced by the gate's own method. Ticking this without re-measuring would have been luck rather than verification, and unticking it — which is what the stale numbers argued for — would have been the same error in the other direction.
            - What was actually raised: `lib/api/client.ts` 61.66 % statements / 41.66 % functions → 100 %, which mattered because it is the RFC 9457 client every page depends on and its untested half was the token refresh and the once-only 401 retry. `query-keys.ts` 64.7 % → 100 %, asserting the two cache-correctness properties its own comments claim — one of which is a cross-tenant read if it fails. Thresholds raised 70/70/70 → statements 90, lines 90, functions 90, branches 80: a floor of 70 against 95.81 % permits 25 points of silent decay, which is not a gate. Proven to bite — one test file alone gives `ERROR: Coverage for lines (11.54%) does not meet global threshold (90%)` and exits 1.
...
- **The 166 leaves contained no leaf for the browser reaching what they built, and that is the plan's own gap rather than an implementation failure.** Every group here is organised around a design section, and design sections describe the engine: routers, chokepoint stages, migrations, properties. `phases.md` §1.2, §1.4, §1.6, §1.7 and §1.8 each also name a FRONTEND deliverable, and seven of those had no leaf and no screen — so the plan could be executed to completion, with all 31 properties proved and all five workflows green, while the backend served 48 routes and the frontend called 13. A plan whose units are design sections cannot notice that, because nothing in it is a unit of "a user can do this".
- **What the closing pass added, recorded here rather than as new leaves.** Four backend prerequisites that four of the frontend deliverables were impossible without: `GET /api/v1/policies` (a list route the module never had, so every other policy operation needed an id nothing would return); project archive and delete for PRD FR-05, with the cascade counted and the append-only audit rows preserved by the design revision `0007` already recorded; tags and per-user favourites for FR-02 and FR-03 with server-side filtering, which needed revision `0013` because `projects.settings.favourite` is per project and a favourite belongs to a person; and the authenticated principal's role on `POST /auth/refresh`, without which the UI could not model the three role gates and a button's first feedback was a 403. Then the seven frontend deliverables, ten unsurfaced capabilities, and `frontend/e2e/onboarding.spec.ts` — ten steps from an empty database to an applied change set, by clicking.
- **One fabrication in the plan's own territory.** `POST /policies/{id}/test` returned `"allow" if input["action"] == "allow_me" else "deny"` when the `opa` binary was absent — a verdict on a security surface that no policy engine computed — and the integration test covering it asserted exactly those synthesised values, so it passed identically with and without OPA installed. That is the shape §0.4's regime exists to catch and did not, because a test can only be non-vacuous about a branch it distinguishes. The route raises `503 dryrun-unavailable` now, the binary is in the backend image and the backend CI job, and a separate case monkeypatches `shutil.which` so the absent-binary path is exercised on every machine including the ones that have OPA.
