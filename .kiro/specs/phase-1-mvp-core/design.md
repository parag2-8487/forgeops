...
| `cerbos` | `==0.14.0` | Apache-2.0 | Tech-Stack §9, PRD §5 | Cerbos client for resource-scoped authorisation |
...
| `cryptography` | `==44.0.0` | Apache-2.0 / BSD | §3.1 internal CA, §11.8 local sealing | Already present transitively via `pyjwt[crypto]`; promoted to a direct pin because Phase 1 uses it directly for X.509 issuance and AES-256-GCM |
