...
- [ ] 7. Build the governance chokepoint and the mutation boundary before any mutating operation
  ...
  - [ ] 7.3 Implement the chokepoint reachability check and test it

    - **Resequenced to run after 7.6.** This leaf's own non-vacuity rule makes it unbuildable in its original position. §2.2.1 requires the check to `exit 1` when the discovered primitive set is **empty**, and the set is discovered by scanning `backend/src/**` for the `@mutation_primitive` decorator. After 7.1 that decorator has **zero** uses — `grep -rn '@mutation_primitive' backend/src/` finds only its own docstring — so the check would correctly refuse to pass on a correct tree, and wiring it into the `agent` and `backend` jobs and `pre-commit` here would red the build until 7.5 and 7.6 land. The first true primitives, by the definition in `governance/primitives.py` ("changes state outside this process"), are `AuditWriter.append` (7.6) and the chokepoint's apply and revert paths (7.5); envelope signing is **not** one, because it produces bytes and mutates nothing. Same disposition as leaves 2.5 and 4.2: the leaf's wording is what blocked it, not its work.
  ...
  - [ ] 7.10 Write property test Q-01 for atomic all-or-nothing application
    ...
    - _Design: §10.5, §7.11(f), Appendix A.9, Appendix B Q-01; Deliverable: 1.6; Criterion: 6; Property: Q-01_
