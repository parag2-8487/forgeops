 ...
 * handling, because the browser must authenticate at the IdP rather than here — sending a password
 * to this origin would make the application a credential intermediary, which is the whole thing
    ...
    <main className="flex min-h-dvh items-center justify-center p-6">
      <div className="w-full max-w-md space-y-6 rounded-lg border border-border bg-background p-8">
        ...
        <p className="text-xs text-muted-foreground">
          You will be returned to <code>{next}</code> once the identity provider has verified you.
          No password is ever sent to this application: it receives an authorization code, exchanges
          it server-side, and never sees your credentials.
        </p>
      </div>
    </main>
  );
