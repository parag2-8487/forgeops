...
/** Signs in through the IdP's own login form. No session injection: step 1 is the real flow. */
async function signIn(page: Page) {
  await page.goto("/");
  // AuthBoundary redirects an anonymous visitor here.
  await page.waitForURL(/\/login/, { timeout: 30_000 });
  await page.getByRole("button", { name: /single sign-on/i }).click();

  // Authentik's identification stage, then its password stage. Two pages, not one.
  await page.waitForURL(/\/(if|application)\//, { timeout: 60_000 });
  await page.getByRole("textbox").first().fill(OPERATOR.username);
  await page.keyboard.press("Enter");
  await page.locator('input[type="password"]').fill(OPERATOR.password);
  await page.keyboard.press("Enter");

  // Back in the application, with a session established by the callback's cookie.
  await page.waitForURL((url) => !url.pathname.startsWith("/login"), { timeout: 60_000 });
}

test.describe("Criterion 10: the end-to-end journey", () => {
  test("step 1 — log in through the OIDC issuer", async ({ page, request }) => {
    test.skip(
      OPERATOR.password === "",
    ...
    const sessions = await eventually("a session row for the operator", () => {
      const count = sqlScalar(
        `SELECT count(*) FROM sessions s JOIN users u ON u.id = s.user_id
         WHERE u.email LIKE '%${OPERATOR.username}%' AND s.revoked_at IS NULL`,
      );
    ...
    const whoami = await apiGet(request, "/projects?limit=1");
    expect([200, 403]).toContain(whoami.status());
  });

  test("step 2 — create a project pointing at the fixture app", async ({ request }) => {
  ...
  test("step 3 — mint a pairing code and pair the real agent", async ({ request }) => {
    const minted = await request.post(`${API}/agents/pairing-codes`, {
  ...
  test("step 4 — the device is active and heartbeating", async ({ request }) => {
    ...
    const listed = await apiGet(request, `/agents/devices/${journey.deviceId}`);
  ...
  test("step 5 — the readiness score renders with a category breakdown", async ({
    ...
    const report = await apiGet(request, `/projects/${journey.projectId}/readiness`);
  ...
  test("step 6 — generate the Dockerfile and Kubernetes manifests", async ({ request }) => {
    const response = await request.post(`${API}/generation/runs`, {
      data: {
  ...
  test("step 8 — the change set shows a diff with both view modes working", async ({
    ...
    const detail = await apiGet(request, `/approvals/${journey.changeSetId}`);
  ...
  test("step 12 — the audit viewer lists the full transit with actors", async ({
    ...
    const events = await apiGet(request, "/audit/events?limit=100");
  ...
  test("step 13 — revert, and every file returns to its pre-image byte-for-byte", async ({
    ...
    const reverted = await request.post(`${API}/approvals/${journey.changeSetId}/revert`, {
      data: {},
    });
