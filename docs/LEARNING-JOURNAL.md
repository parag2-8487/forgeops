...
### D-79 — the hub inherits the placeholder's refusal instead of its absence

`UnavailableCommandSink` was the composed `CommandSink` until leaf 8.4, and it refused every
delivery with a 409 `device-not-connected`. Replacing it with a real hub is the moment that refusal
could quietly disappear — and the disappearance would be invisible, because every test that used to
assert the refusal would now be asserting the hub's happy path.

So `AgentHub.send_command` checks `forgeops:agentsession:<device_id>` before it enqueues anything and
raises the **same** problem type when the key is absent. The key's TTL is the heartbeat timeout, so
"connected" means "beat inside the window" rather than "connected once". A Redis error is also a
refusal: `_has_live_session` returns `False` on any exception, which is the same direction every
other unavailability in that module takes.

What was rejected is the shape that reads as more capable: enqueue unconditionally and let the
stream hold the command until the device returns. It was rejected because every envelope carries
`not_after` (§7.6), so a command queued for an absent device is expired by the time it is delivered,
and the agent would reject it as `envelope-expired` — a rejection indistinguishable from an attack.
Worse, the transit that minted it would have reported `applying` with nothing in flight, leaving the
change set in a state no operator can act on. Refusing keeps the set `approved` and retryable, which
is what the placeholder's docstring already promised.

The second half of the decision is that delivery goes through the Redis stream **even when this
replica owns the socket**. A local fast path is obviously cheaper and was rejected: it would be a
second delivery implementation with its own ordering, and two orderings for one device is a
duplicate-or-reordered `seq` that the agent cannot distinguish from a replay. The cost is one Redis
round trip on the common path, paid to keep one code path.

What it costs overall: the liveness check is a Redis read on the delivery path, and a device whose
heartbeat is late by more than the timeout is treated as absent even though its socket is still
open. That is the same window the hub already uses to drop sessions, so it is one window rather than
two — but it does mean a delivery can be refused for a device that would have answered.

### D-80 — certificate rotation rides `session.heartbeat`, because the catalogue is closed

§3.1 requires the device certificate to be renewed **over the live session**, so a reconnect never
needs re-pairing, and leaf 8.2 built `rotate_certificate` for it. §7.3 fixes **nine** JSON-RPC
methods. Those two facts collide: the obvious carrier for a renewal is a `session.rotate` method,
and adding it would make the catalogue ten.

The decision is that an agent asks for rotation by including a `csr` in its `session.heartbeat`
params, and the hub returns a `certificate` member in the heartbeat result. No method is added, and
the request rides a frame the agent is already sending every thirty seconds — which also means a
device that cannot heartbeat cannot rotate, and a device that cannot rotate is one whose certificate
is about to expire anyway.

What was rejected, and why the rejection is not pedantry: a tenth method. The value of a closed
catalogue is that a reviewer can enumerate every message that exists by reading one table, and the
first exception is the one that makes the table advisory. There is also a REST route one could
imagine instead, and §11.2 already rejected that for a stronger reason — a device certificate handed
out over a route authenticated by anything other than the device's _current_ certificate would be a
second, weaker enrolment path.

What it costs: `session.heartbeat` now has two jobs, and its handler is the longest one in the
dispatch table. A refused rotation is reported **in the result** (`{"rotated": false, "reason": …}`)
rather than raised, because a certificate problem must not become a dropped socket and then a
reconnect loop — so a caller that ignores the result member cannot tell that rotation failed. That is
...
### D-82 — the session names its executor, and the reset is to the base rather than to zero

Two decisions from leaf 8.5, both about the shape of a seam rather than about an algorithm.

**The runner is an interface this package declares.** `Serve` has to hand a verified command to
something that executes it, and that something — `executor.Dispatcher` — does not exist until leaf
8.7. Three answers were available: build 8.7 first, import a type that is not there yet, or let the
consumer name what it needs. The third was taken, so `session.CommandRunner` takes a
`*envelope.Verified` and returns a `CommandOutcome` this package owns. The argument type is the point:
only `envelope.Verify` can construct a `Verified`, so the **interface itself** carries the rule that
nothing unauthenticated reaches an operation, and a future dispatcher cannot accidentally offer a
second entry point. It also matches what leaf 8.4 did on the backend side, where the hub declares the
`DeviceDirectory` protocol it needs instead of importing `DeviceService`.

What it costs, stated plainly: `executor.Dispatcher`'s own signature (§10.5) is
`Execute(ctx, v, sink ProgressSink) (Result, error)` with `Result` and `ProgressSink` in `executor`,
so the two do not match by construction and the app wiring will need a small adapter. That is a real
cost and it buys leaf ordering — 8.5 lands and is tested before 8.7 exists, rather than the two
becoming one large unverifiable change.

**Reset means "start over at the base", not "retry immediately".** §7.4 says the reconnect counter
resets only after a successful `session.connect`. The first implementation read that as `attempt = 0`,
which is what the counter starts at — and attempt 0 is deliberately not delayed, because the first
dial of a run should be immediate. Writing the control for the hot-loop test is what exposed it: a
backend that **accepts** the handshake and then drops the socket would have produced a reconnect loop
with no delay at all. That is the same defect the rejected-handshake rule exists to prevent, arriving
one step later and looking like success. So an accepted session that ends sets `attempt = 1`, whose
delay is the base second. The lesson is the one chapter 9 keeps re-learning: the control found the
bug, not the assertion.

### D-83 — the `approval_id` requirement moves to where operations are known, and gets stronger

`envelope.parse` required a non-empty `approval_id` for **every** operation. §7.7's table requires
one only for the mutating half, so the check refused valid envelopes: the backend legitimately
sends `scan.full` with the member present and empty, and the agent would have answered
`envelope-malformed`. Group 7 recorded the over-strictness deliberately, as wrong-by-refusal being
the safer direction to be wrong in until the catalogue existed.

The catalogue exists now, so the check moved to `executor.Execute`. Three options were on the
table. Keep it strict — rejected, because it refuses commands the backend sends today. Relax it and
stop there — rejected, because that is a pure weakening. Relax it and put a stronger rule where the
operation is known — taken.

The replacement is stronger in a specific, checkable way. Each row of the dispatch table declares
`mutating` and `requiresApproval`, and two tests hold them together: every mutating operation must
require an approval, and every operation that requires one must mutate. Then `Execute` refuses a
mutating operation whose `approval_id` is empty, before it decodes a single argument. The blanket
rule could not distinguish `scan.full` from `changeset.apply`; this one distinguishes exactly the
cases §7.7 distinguishes.

Why it cannot live in `envelope`: that package imports nothing from `internal/**` (D-59, asserted
by `TestPackageIsALeaf`), so it cannot consult the catalogue, and it should not — the checks it owns
are the operation-independent ones. Schema, freshness, signature, ordering and uniqueness are true
of an envelope; "needs an approval" is true of an operation.

**The cost, and it is a real narrowing.** `*envelope.Verified` no longer implies "carries an
approval". Its guarantee is: signed by the device's envelope key, fresh, in order, unique, and bound
to the policy bundle this agent holds. Anything relying on the broader reading has to say so
explicitly. Grepped: `mutate.ApplyVerified` and `mutate.Revert` take a `*Verified` as proof of
authority and never read `ApprovalID`, and the executor is now the only path to either — so the
approval check sits in front of both. `session` reads `ApprovalID()` nowhere. Nothing else consumes
the type.

One thing improved for free: the corpus fixture `08-non-mutating-no-approval` had to be excluded
from `TestCorpus_VerifyAcceptsTheCommittedSignature` because of the old rule. It now verifies like
any other fixture, so the cross-runtime agreement covers one more shape without a fixture being
written.

### D-84 — the policy-binding check runs before the two checks that mutate

Appendix A.2 numbers the checks freshness, signature, ordering, uniqueness, policy binding — and
states the postcondition "no mutation is performed on any failure path". Implemented in that order,
the two are in conflict: ordering is a compare-and-set and uniqueness records the nonce, so a
**stale-bundle** rejection — which is not a mutating check at all — consumed the envelope's `seq`
and burned its nonce on its way out. A rejection that changes state is exactly the shape Q-15
forbids.

A.2's own pseudocode resolves it by deferring both mutations to the end, after every check has
passed. Two ways to get there. Split `ReplayGuard` into peek-then-commit — rejected: the
compare-and-set is what makes ordering atomic under concurrency, and §7.6 makes the backend's copy
of this state Redis-authoritative through a Lua CAS for precisely that reason. Or move the
non-mutating check in front of the mutating pair — taken. The numbered order is not itself a
property; the postcondition is, and the one ordering constraint that _is_ a property —
signature before any state change — is untouched.

What it costs: the implementation no longer reads in A.2's numbering, so the docstring has to say
why, and it does. And one residual is left, recorded rather than papered over: an **authenticated**
envelope carrying a fresh `seq` and a reused nonce still advances the high-water mark, because
ordering runs before uniqueness and the set is the check. `TestVerify_AUniquenessRejectionDoesAdvanceSeq`
asserts that as the documented behaviour and says why it is harmless — the caller is authenticated,
so it is not a lever an attacker has, and the backend allocates `seq` monotonically, so no
legitimate envelope is lost.

### D-85 — a catalogued operation with no body reports its own code, and the table carries §7.7's columns

Two decisions about the shape of the dispatch table, both about not having the same fact twice.

**`operation-unimplemented` is not `operation-unknown`.** Fifteen of §7.7's seventeen operations
have no body in this phase; their implementations belong to groups 10 to 15. The options were a
missing table key, which reports `operation-unknown`, or a named refusal. Named refusal taken:
"we do not have that operation" is a version skew and "we have it and its body arrives later" is a
decomposition boundary, and only one of them is a bug. One code for both would send an operator
hunting the wrong problem, and it would make the closed catalogue unreadable from the outside. The
cost is one suffix beyond Appendix C.2's list, which is a deviation from the spec's vocabulary and
is stated here rather than absorbed silently. `OperationInfo.Implemented` reports the same fact to
`agent doctor`, and a test pins the count at **2**, so growth is deliberate and visible in a diff.

**One table row carries the handler and §7.7's three columns.** §10.5 sketches
`map[Operation]handler` with the metadata elsewhere. Two maps keyed by the same enum is journal
pattern H, and the failure mode is specific rather than aesthetic: an operation added to the handler
map and forgotten in the mutating-flags map would be a mutation that skips the approval check.
`Operations()` derives its report from the same table, so `agent doctor` cannot disagree with what
dispatch will do. The cost is a small departure from the design's sketch, and a struct where the
design has a function type.

### D-86 — Q-14's cross-runtime clause is discharged over the committed corpus, not by shelling out

Q-14 quantifies over envelopes and asserts, among other things, that `CanonicalBytes` is
byte-identical **in Go and Python**. That cannot be asserted from inside one runtime, and the
property has to live somewhere.

Three shapes were considered. Generate in Go and shell out to the backend virtualenv per batch —
rejected: the agent CI job has no Python environment, so the test would fail there for a reason
that is not the property, and making it conditional would be a skip in disguise, which §0.4.4
forbids and `scripts/check-no-skips.py --go` enforces. Generate in Python and call a Go helper —
the same objection with the runtimes swapped. Discharge the cross-runtime clause over the
**committed corpus** — taken.

`agent/testdata/envelopes/*.json` holds each envelope together with its expected canonical bytes as
hex and its expected signature under a synthetic key. `corpus_test.go` asserts the Go
implementation against those bytes; `tests/unit/test_governance_envelope.py` asserts the Python one
against the same files. A divergence fails both suites rather than producing a mystery in
production, which is what §7.6 asks for. The generated clauses that _can_ be checked inside one
runtime — determinism, member-order independence, JCS shape, exact acceptance, single-byte
rejection — run in both, over generators of the same declared shape.

**The residual, named because it is the thing a reader would otherwise assume away:** a divergence
that appears only for an envelope shape the corpus does not contain is caught by neither side. The
mitigation is that the corpus covers the corners the two runtimes actually disagreed on while it
was being built — RFC 8785's integer boundary, UTF-16 versus code-point key order, string
escaping, empty containers — and both generators draw from those shapes, including astral-plane
keys. It is a weaker guarantee than a live cross-process comparison and it is the one the CI
topology allows.

Two asymmetries surfaced while writing the two generators, both recorded rather than smoothed:
the Python side **requires** `command_id`, `device_id` and a non-empty `approval_id` to be UUIDs
while the Go verifier requires only non-emptiness; and Python's `signing_input` **refuses** an
empty domain prefix while Go's accepts one. Both lean the safe way — the strict check is on the
minting side, and the permissive one is on the verifying side, which admits nothing a legitimate
signer would produce — and both generators now draw UUIDs so the two properties quantify over one
shape rather than two dialects of it.

### Leaf 8.9 — the generator was wrong before the property was, and that is the usual order

Q-15 quantifies over envelope **streams**, and writing the generator surfaced two things worth
keeping.

The first is a modelling error that read as a property failure. The generator originally positioned
each event's `seq` against the last **accepted** seq. But a rejected nonce replay advances the mark
(D-84's recorded residual), so after one replay the next replay's `seq` was no longer strictly
greater — and that envelope is refused by **ordering**, not by uniqueness. The property was then
asserting the wrong clause about the wrong rejection, and it failed with a message about D-84 having
changed. Positioning against the guard's **live** mark fixed it. The lesson is not subtle but it is
easy to miss: when a rejection has a documented side effect, a generator that models only the
successful path drifts out of step with the system after the first refusal.

The second is smaller and more common: rapid shrinks towards small values and drew nonce `0x0`
repeatedly, so a "fresh" event collided with an earlier one and failed as a replay. A generator bug
that reads exactly like a property failure. The event index is now folded into the nonce, so
freshness is by construction while the rest of the value is still drawn.

What the property adds over the example tests in `rejection_test.go`: those check one refusal at a
time against a clean guard. Only a stream can observe what one envelope leaves behind for the next,
which is the whole subject of §7.6's three replay conditions. The invariant is checked after **every
event** rather than at the end of the stream, so the first offending event is the one reported.

The one honest exception is encoded rather than avoided: the property asserts that a nonce replay
**does** advance the mark and that every other rejection does not. Writing it the tidy way — "no
rejection advances anything" — would have required either a false assertion or a peek-then-commit
guard, and D-84 records why the second was rejected. A property that states the exception is worth
more than one that quietly excludes the case.

`order.go` was extracted for the overlay, for the third time in this repository after `rollback.go`
(Q-01) and `domain.go` (Q-14). The pattern is now explicit enough to name: **when a property's
negative control is "do this in the wrong order" or "leave this step out", the step belongs in its
own file before the control is written.** `go build -overlay` replaces whole files, so the file
boundary determines how readable the control is, and a control nobody can read is a control nobody
maintains.

### D-87 — Q-31's control mutates the vocabulary, and Appendix B was amended to say so

Appendix B's negative control for Q-31 read: "add a `KindEnvelope` case to `Journal.Drain` that hands
the stored envelope straight to `executor.Execute`". That branch cannot be written as an honest
overlay, because it has nothing to consume. D-41's guarantee is that `RecordKind` has no kind capable
of representing an authorisation, so a `Drain` case reading a stored envelope would first need a
stored envelope to exist — which means shipping production code whose only purpose is to be broken: a
real, reachable path from a persisted envelope to the executor, guarded by a test. That is the hole
D-41 exists to prevent, created in order to check that it is prevented. Q-01's row already warns
against controls of that shape.

So the control breaks the fact the property rests on. `journal_kinds.go` is overlaid with a version
that adds `KindEnvelope` and admits it in `validKinds`, and `Append` will then persist a record
carrying an authorisation. It bites **earlier** than Appendix B's version rather than later: the
guarantee has already failed before any drain runs.

Two things were done rather than one. `journal_kinds.go` was extracted — the fourth such extraction
after `rollback.go`, `domain.go` and `order.go`, but the first with a second reason beyond the
overlay mechanics: that table **is** the guarantee, so a reader checking D-41 should be able to check
it by reading one short file. And **Appendix B's Q-31 cell was amended in place**, citing this
decision, the way D-51 and D-57 superseded design text they contradicted. Leaving the appendix saying
one thing while `mutations.toml` did another is journal pattern H with a specific failure mode: the
harness already fails when `mutations.toml` lacks a row for an Appendix B id, so the two agree on
_coverage_ and would have drifted silently on _content_ until nobody knew which control ran.

What it costs: the appendix now carries a parenthetical explaining a superseded wording, which is
prose a first-time reader has to step over. The alternative was a divergence recorded only in a
commit message.

### D-88 — Q-17's control was vacuous, and the fix was to assert which refusal, not to weaken the rule

The Q-17 row landed reporting `VACUOUS`: the property passed with the consume script made
non-atomic. The reason is worth writing down, because it is a fact about the system rather than about
the test. `DeviceService.exchange` has a **second** serialisation point after the Lua `EVAL` — the
`UPDATE … WHERE id = :id AND status = 'pending' AND pairing_token_hmac = :digest`. With the script's
atomicity removed, all concurrent callers pass the consume, and then exactly one `UPDATE` affects a
row; the rest see `rowcount = 0` and are refused. At-most-one therefore survives a non-atomic consume
**on its own**. That is defence in depth working as intended, and it also means the success _count_
alone cannot fail under Appendix B's control.

Three options. Weaken the control to break both serialisation points — rejected: a control that
breaks two things at once stops being a diagnostic. Redefine Q-17's control — rejected: Appendix B
names the non-atomic consume, and the point of the appendix is that the control is fixed in advance.
Or find the clause that _is_ sensitive to the consume specifically — taken.

It is the refusal **reason**. With one atomic `EVAL` the losing callers never observe the key and are
refused as `missing`, writing no `device-not-pairable` audit record. With a read-then-delete pair they
all pass the script and pile up on the row, so every loser writes one. The property now asserts that
no `device-not-pairable` record exists after a batch of concurrent attempts on a fresh code — and the
test's refusal path had to start **committing** instead of rolling back, because the rollback was
discarding exactly the evidence that distinguishes the two cases.

The general lesson, which is the third time this session it has come up: a property that only counts
outcomes cannot see a mechanism that has been replaced by a different mechanism with the same
outcome. Asserting _why_ something was refused is what pins the mechanism.

### D-89 — finding 65's proof is owed against leaf 18.1, not against "later"

Finding 65 — Phase 0's transport sending binary frames the hub's `receive_json()` cannot read — is
fixed on both sides and proved by nothing. Both halves agree in source, and that agreement is a
reading of two files rather than a test. It has been recorded as owed for two leaves now, which is
how a debt becomes permanent.

Proving it requires a real WebSocket frame crossing from a real Go binary to a real FastAPI hub.
Neither existing suite can do that: the hub's tests drive a fake socket by design, and the agent's
drive a fake transport. Building a one-off harness for it would mean a second e2e stack — a uvicorn
process, a built agent binary, a paired device — living beside the one **leaf 18.1** already exists to
build, which runs the real `forgeops-agent` binary against the real stack and pairs it with a code
minted through the API.

So the debt is assigned there rather than left general. What it costs: the frame mode stays unproved
until group 18, which is late in the phase, and the mitigation is only that a mismatch fails loudly
...
### Pattern T — output verified by substring, so the bytes around the substring were never read

**Finding 62.** `agent/internal/app/commands.go` printed `agent doctor`'s status lines with
double-encoded tick and cross glyphs: the source bytes decode as `âœ“` rather than `✓`, because a
correctly encoded `✓` (`E2 9C 93`) had at some point been read as CP1252 and re-encoded as UTF-8.
Every line of `forgeops-agent doctor` output therefore began with mojibake, on every platform, for
as long as the command has existed.

Nothing caught it, and the reason is the pattern rather than the encoding. `app_test.go` asserts
that doctor's output _contains_ `Docker:`, `Kubernetes:` and `OpenTofu:` — substrings the test
author wrote and therefore substrings that are right. The glyph is the part nobody wrote a literal
for, so no assertion covered it, and no reviewer read the diff hunk closely enough to notice that
three bytes were seven. A test that asserts on the fragments it authored can only ever verify the
fragments it authored; the space between them is unverified by construction.

The axis this names, against its neighbours: pattern I is a tool that gates the build and is itself
verified against nothing; T is _output_ verified against a projection of itself. The distance
matters because the remedy differs. For I the remedy is a fixture that makes the tool fail. For T
there is no test to add that would generalise — asserting the exact glyph is just one more authored
literal — so the remedy is to remove the class of mistake instead: the glyphs are now `\u2713` and
...
What it cost: nothing beyond the ugly output, because the glyph carries no meaning a machine reads.
That is exactly why it survived: a defect whose only victim is a human's eyes has no failing test to
write.

### Pattern U — the check that cannot execute on the platform its operator uses

**Finding 63.** The `go-vet` pre-commit hook was declared `language: script` with
`entry: scripts/go-vet-changed.sh`, which asks pre-commit to run the file through its shebang. On
Windows there is no `/bin/sh`, so the hook aborted with `Executable /bin/sh not found` — meaning
`go vet` had never once run through the local gate on this machine. CI runs on Linux and passed, so
the two environments disagreed about whether the check existed.

It stayed invisible because the hook is filtered to `^agent/.*\.go$` and no session on this branch
had changed an agent Go file while running the gate. The first leaf that did — 8.3 — turned it red
immediately. That is the honest reading, and it is worth stating precisely: this was **not** a gate
that passed while checking nothing (pattern B). It was a gate that would have failed loudly the
first time it was reached, and was simply never reached. The damage is the same shape as B's — a
check nobody was actually running — but the failure mode is different, and so is the fix.

The fix is `entry: bash scripts/go-vet-changed.sh` under `language: system`, which is the spelling
three sibling hooks in the same file (`check-no-latest`, `check-gitleaks-config`,
`check-chokepoint`) already use and which works on both platforms. So the repository already had the
answer, in the same file, eleven lines away. That is the part worth generalising: **when one hook in
a config is declared differently from its siblings, the difference is a defect until somebody writes
down why it is not.**

The second general rule: a filtered hook is only as verified as the last time its filter matched. A
gate that runs on every commit is tested continuously by being run; a gate scoped to a file pattern
is tested only when that pattern is touched, so its first genuine invocation can be months after it
was written. Both `gofmt` and `go-vet` carry `files: ^agent/.*\.go$`; `gofmt` is declared
`language: system` and has always worked, which is why the divergence produced no symptom until now.

What it cost: `go vet`'s findings were still caught, because `go vet ./...` is run directly as part
...
### Pattern P again — finding 65, two halves of one protocol that had never exchanged a frame

**Finding 65.** Phase 0's `WSSTransport.Send` wrote every payload as a **binary** WebSocket frame.
The backend hub reads every frame with Starlette's `receive_json()`, whose default `mode` is `"text"`
and whose implementation is `text = message["text"]` — checked by reading
`starlette/websockets.py` in the installed virtualenv rather than recalled. A binary frame's ASGI
message carries `bytes` and no `text`, so the hub would have raised before parsing a single byte of
JSON. The agent would have connected, sent `session.connect`, and been dropped.

Nothing caught it because nothing could. Phase 0's transport never dialled a live backend — its
`Manager.Serve` was a stub with a comment saying the read loop arrives in a future phase — and the
hub's own tests (leaf 8.4) drive a **fake** socket whose `receive_json` is a Python callable, which
is the right test for the properties that leaf asserts (revocation per frame, cross-replica delivery)
and structurally incapable of noticing the frame type. Each half was tested against its own idea of
the contract. That is pattern P, and the variant is worth naming: **the seam was not
under-tested, it was untested on both sides by construction**, because neither side's test involves a
real WebSocket frame.

The fix is one word, `MessageText`, and it is a defect fix rather than a preference: every payload
this transport carries is UTF-8 JSON-RPC (§7.3), which is what a text frame is for. Two additive
changes came with it, and both were kept additive on purpose. `NewWSSTransport` gained variadic
options so `WithTLSConfig` can supply the client certificate the hub's two-secret handshake requires —
Phase 0 dialled with the default HTTP client, which presents none, so mTLS was not expressible through
the existing seam at all. And `connection.CloseStatusOf` exports the close code, because §3.1 gives
4403 the meaning "this device is revoked" and the session manager has to wipe credentials on it rather
than count it as one more failed connection. The `Transport` **interface** is untouched, which is what
§10.3's "consumed, not modified" has to mean in practice: existing call sites still compile.

What it cost, and what is still owed: the frame type is now asserted nowhere against a real socket.
Both halves agree in source, and that agreement is a reading of two files rather than a test. The
honest way to close it is one end-to-end case that runs the real Go transport against the real
FastAPI hub, which needs both runtimes in one harness; it belongs with group 8's close-out or with the
Q-31 row, and until it exists this finding is fixed but not **proved** fixed.

### Pattern B again — finding 66, three checks whose exit status was `tail`'s

**Finding 66.** `scripts/leaf-gate.ps1` invoked its three shell checks as
`bash -c "... && bash scripts/X.sh 2>&1 | tail -3"`. The exit status of a pipeline is its **last**
command's, so the status recorded for `check-hygiene.sh`, `check-structure.sh` and
`check-makefile.sh` was `tail`'s, which is always 0. All three could fail and the gate would print
`exit=0`, add nothing to its failure list, and report `all static checks clean`.

This was not theoretical. During leaf 8.5 the new `check-added-shapes` hook was not yet in
`check-hygiene.sh`'s exempt list, so the hygiene check failed with two violations — and the gate
printed the violations two lines above `check-hygiene.sh exit=0` and then declared the tree clean.
The failure was **visible and uncounted**, which is worse than invisible: it teaches the reader that
the words in the output matter more than the status, and the next such failure will be skimmed.

The fix is `bash -o pipefail`, proved both ways rather than assumed:
`bash -o pipefail -c "bash -c 'exit 3' | tail -3"` exits **3**, and the same command without
`-o pipefail` exits **0**. The `| tail -3` is worth keeping — these checks print a long preamble and
only the verdict matters — so the fix is the flag rather than dropping the pipe.

Two general points. First, this is pattern B in its purest form: a correct check, wired so that its
verdict could not reach the person reading the output. The check was not weak; the plumbing was.
Second, it is the third time in this branch that a gate's **wiring** rather than its **logic** was the
defect — finding 63 (a hook that could not execute on this platform), finding 64 (a check positioned
...
|                             |  Leaves |
| :-------------------------- | ------: |
| `done`, with cited evidence |  **66** |
| `blocked`                   |       0 |
| `pending`                   |     100 |
| **Total**                   | **166** |

`PROGRESS.md` is the authority for this table. It is verified mechanically after every leaf by
`scripts/_state.sh`, which reports `done 66`, `pending 100`, `TOTAL 166` over the Phase 1 section
...
| 8 · Pairing, session protocol, named-operation executor                  |     12 | **complete, including its close-out** — 8.1 through 8.12 done. 8.1: pairing-code issue and single-use exchange (D-70, D-71, D-72; finding 55). 8.2: the internal CA, `make init-ca` and certificate rotation (D-73 — the CA discards the CSR's subject and the fingerprint, not the chain, is the authorisation input; D-74; findings 56 and 57). 8.3: the agent's `pair` command, `session.Manager.Pair`, credential persistence through the keychain-or-0600 store, and `agent doctor` telling "no backend URL" (`connection.ErrDisabled`) apart from "no device token" (`session.ErrUnpaired`); `Pair` refuses locally before it sends, so a mistyped second `pair` cannot unpair a healthy agent; finding 62 fixed the doctor output's double-encoded glyphs. 8.4: the WebSocket hub at `/api/v1/ws/agent` — two-secret handshake, revocation checked **per inbound frame** and failing closed, cross-replica delivery over a Redis stream, results correlated back over pub/sub, D-79 (the hub keeps the placeholder's `device-not-connected` refusal) and D-80 (rotation rides the heartbeat rather than a tenth method); finding 63 found that `go vet` had never run through the local gate on Windows. 8.5: the agent's own session loop — `Serve`, the 30 s heartbeat with its 90 s deadline enforced in both directions, §7.4's backoff whose counter resets only on a successful `session.connect` and then to the **base** rather than to zero (D-82), the journal drain gated behind connect and the bundle check with intents replayed as `approval.request` (D-41), and revocation acted on from either an `agent.error` frame or a 4403 close by wiping the journal and the credentials; finding 65 found that Phase 0's transport sent binary frames the hub's `receive_json()` cannot read. 8.6: the ±60 s measured clock skew, taken from the handshake's `server_time` — the only reference point the protocol carries — reported in `agent.status` and in `doctor`, and refusing commands with `clock-skew` before verification so the agent names the clock rather than the envelope; the rejection-path table proving that nothing refused before the two mutating checks advances a counter (D-84 reordered the policy check to make that true); and D-83 moved the `approval_id` requirement out of `envelope`. 8.7: `executor.Dispatcher` with §7.7's seventeen operations in one closed table, per-operation timeouts, progress emission, the approval requirement D-83 relocated, and a source-parsing test asserting no handler is reachable outside the table; two operations have bodies (`changeset.apply`, `changeset.revert`, both through the compile-enforced `mutate` boundary), the other fifteen name the group that brings theirs (D-85). 8.8: Q-14 in both runtimes, with the cross-runtime half discharged over the committed corpus rather than by shelling out between them (D-86), `domain.go` extracted so the negative control is a one-string overlay, and two implementation asymmetries recorded. 8.9: Q-15 over generated envelope streams, with the invariant checked after every event, the one documented exception asserted rather than excluded, and `order.go` extracted for the control. 8.10–8.12: Q-16 over generated revocation timings, Q-17 over generated concurrency and refusal order, Q-31 over generated journal contents — `mutations.toml` reaches **14 of 31**, and D-87, D-88 and D-89 came out of writing them. Close-out: the full fourteen-row sweep, all fourteen also proved attributable by the new `scripts/control-of-the-control.py`, `pytest -m mandatory` **1568 passed**, `go test -race -shuffle=on ./...` clean after it found real races in leaf 8.5's test doubles, and the comprehension artifact regenerated |     |
