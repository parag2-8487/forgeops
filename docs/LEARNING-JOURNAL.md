...
| Leaves reflected       | **61 of 166** `done` in `PROGRESS.md`, 0 `blocked`, 105 `pending`. Reconciled 2026-08-02; all three sources agree via `scripts/_state.sh`. **Group 7 is complete** — all eleven leaves plus its close-out (`verify-chain` proved end to end, comprehension artifact regenerated). **Group 8 is in progress**: 8.1 through 8.7 are done — pairing-code issue and single-use exchange, the internal CA, the agent `pair` command, the backend WebSocket hub, the agent session loop, envelope verification with its measured clock skew, and the named-operation dispatch table. Decisions run to **D-85**; findings run to **66**, and patterns T and U are new. See chapter 10. |
...
|                             |  Leaves |
| :-------------------------- | ------: |
| `done`, with cited evidence |  **61** |
| `blocked`                   |       0 |
| `pending`                   |     105 |
| **Total**                   | **166** |

`PROGRESS.md` is the authority for this table. It is verified mechanically after every leaf by
`scripts/_state.sh`, which reports `done 61`, `pending 105`, `TOTAL 166` over the Phase 1 section
